...
try:
    data = data.DataReader(symbol, 'google', past_month.strftime("%Y-%m-%d"))
except:
...
for days in [5, 10, 15, 20, 25]:
    ...
    variables_dict['BIAS' + str(days)] = (data['Close'][-1] - variables_dict['MA5']) / variables_dict['MA' + str(days)]
    variables_dict['PSY' + str(days)] = data['Close'][days*(-1):].sum()
    variables_dict['ASY' + str(days)] = SY[days*(-1):].mean()
for days in [1, 2, 3, 4, 6, 7, 8, 9]:
    variables_dict['ASY' + str(days)] = SY[days * (-1):].mean()
...
for key, value in responses.items():
    print('{} probability of price going up: {}'.format(key, value[0,1]))